...
test("day-05/part-2", async () => {
    const input = await fs.readFile(
        path.join("static/input", "day-05", "4"),
        "utf-8"
    );

    const expected = ~~(await fs.readFile(
        path.join("static/expected", "day-05", "4"),
        "utf-8"
    ));
...
test("day-05/part-2", async () => {
    const input = await fs.readFile(
        path.join("static/input", "day-05", "5"),
        "utf-8"
    );

    const expected = ~~(await fs.readFile(
        path.join("static/expected", "day-05", "5"),
        "utf-8"
    ));
