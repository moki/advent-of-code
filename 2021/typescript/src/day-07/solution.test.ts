...
test("day-07/part-1", async () => {
    const input = await fs.readFile(
        path.join("static/input", "day-07", "1"),
        "utf-8"
    );

    const expected = ~~(await fs.readFile(
        path.join("static/expected", "day-07", "1"),
        "utf-8"
    ));
...
test("day-07/part-2", async () => {
    const input = await fs.readFile(
        path.join("static/input", "day-07", "2"),
        "utf-8"
    );

    const expected = parseInt(
        await fs.readFile(path.join("static/expected", "day-07", "2"), "utf-8")
    );
